...
void InitVisibility() {
	...
	//glEnable(GL_DEPTH_TEST);	// 5. 모든 자료를 초기화한 후, depth test를 제거. 그리는 순서대로 누적됨 ////깊이 버퍼를 활설화하는것, 안쓰면 그냥 덮어써서 그림
}

//// 표준적인 방법: 점들이 감긴 방향이 반시계 방향이어야 한다.
//// 시계방향으로 감아서 데이터를 만들어두었다면, openGL에서는 이를 처리하기 위해 option으로 만들어놓았다. glFrontFace(GL_CW);
void MyDisplay() {
